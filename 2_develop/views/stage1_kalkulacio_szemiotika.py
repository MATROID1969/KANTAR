...
# A "kiegészítendő" elemek – önállóan nem megrendelhetők, mellettük legalább
# még egy másik paraméternek > 0-nak kell lennie.
DEPENDENT_ITEMS = {
...
def _validate_dependents(values: dict) -> list[str]:
    """Visszaadja azon függő elemek listáját, amelyek > 0 értékűek,
    de nincs mellettük másik > 0 paraméter."""
    errors: list[str] = []
    other_positive = any(
        (values.get(k) or 0) > 0
        for k, _, _, _ in FELDOLGOZAS_PARAMS
        if k not in DEPENDENT_ITEMS
    )
    for key, label in DEPENDENT_ITEMS.items():
        v = values.get(key) or 0
        if v > 0 and not other_positive:
            errors.append(label)
    return errors
...
def _render_summary_panel(values: dict):
    ...
    for key, label, unit, _ in FELDOLGOZAS_PARAMS:
        ...
        row_bg = "#f4f5f7" if is_zero else "#ffffff"
        label_color = "#a0a8b5" if is_zero else "#3a4658"
        value_color = "#a0a8b5" if is_zero else "#0b3d91"
        unit_color = "#bcc3cf" if is_zero else "#8a96a8"

        rows.append(
            "<div style='display:flex; justify-content:space-between; "
            f"padding:0.5rem 0.75rem; border-bottom:1px solid #eef0f4; "
            f"background:{row_bg};'>"
            f"<span style='color:{label_color}; font-size:1.0rem;'>{label}</span>"
            f"<span style='color:{value_color}; font-weight:600; font-size:1.05rem;'>"
            f"{v_str} <span style='color:{unit_color}; font-weight:500; "
            f"font-size:0.95rem;'>{unit}</span>"
            "</span></div>"
        )
    alap = _calc_feldolgozasi_alap(values)
    ...
    rows.append(
        "<div style='display:flex; justify-content:space-between; "
        "padding:0.6rem 0.75rem; margin-top:0.4rem; background:#eef3fb; "
        "border-radius:6px;'>"
        "<span style='color:#0b3d91; font-weight:600; font-size:1.0rem;'>"
        "Feldolgozási alap (automatikus)</span>"
        f"<span style='color:#0b3d91; font-weight:700; font-size:1.05rem;'>{alap_label}</span>"
        "</div>"
    )
    st.markdown(
        "<div style='border:1px solid #d6dde8; border-radius:8px; padding:0.9rem; "
        "background:#ffffff; margin-top:1rem;'>"
        "<div style='font-size:0.9rem; color:#0b3d91; font-weight:600; "
        "letter-spacing:0.04em; text-transform:uppercase; margin-bottom:0.7rem;'>"
...
def render_stage1_kalkulacio_szemiotika(offer_id: int, is_editable: bool, db: Session):
    ...
    # Validáció: a függő elemekhez (Management summary, Prezentáció)
    # legalább még egy másik > 0 értékű paraméter kell.
    dep_errors = _validate_dependents(values)
    for label in dep_errors:
        st.warning(
            f"A „{label}” önállóan nem rendelhető meg – legalább egy másik "
            "paraméterhez (Debrief, Top-line, Egyszerű elemzés, Mélyelemzés, "
            "Filmkészítés vagy Workshop) is adj meg 0-tól eltérő értéket."
        )

    col_save, _ = st.columns([2, 5])
    if is_editable:
        save_disabled = bool(dep_errors)
        if col_save.button(
            "Mentés",
            type="primary",
