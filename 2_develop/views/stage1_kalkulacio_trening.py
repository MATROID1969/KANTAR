"""
Szakasz 1 – Kalkuláció: Tréning.

Az Excel (1_documents/trening.xlsx) alapján képezzük le a paramétereket.

Blokkok:
  1) Kantar márkázott termékek  (Van/Nincs – részletek később)
  2) Szervezés                  (Van/Nincs – részletek később)
  3) Terepmunka                 (Van/Nincs – részletek később)
  4) Feldolgozás                (Elemzői munkaóra – szabad szám)
  5) Plusz szolgáltatások
  6) Egyéb
...
def calc_munkaora(p: dict) -> dict:
    ...
    # TODO: Feldolgozás (Elemzői munkaóra) – óraszámok még hiányoznak
...
def _render_feldolgozas(p: dict, is_editable: bool, kp: str):
    ...
    with st.expander("4) Feldolgozás", expanded=False):
        ...
        f["elemzoi_munkaora"] = st.number_input(
            "Elemzői munkaóra",
            min_value=0.0,
            step=0.5,
            value=float(f.get("elemzoi_munkaora") or 0),
            key=f"{kp}_feld_elemzoi",
            disabled=not is_editable,
            help="Az ajánlatkérő becsüli az óraszámot.",
        )
...
def _render_munkaora_panel(h: dict, extra_rovid: bool = False):
    ...
    panel_html = [
        ...
        f"<div style='font-size:0.8rem; color:#5a6b80; margin-bottom:0.5rem;'>Tréning{badge_extra}</div>",
...
def _render_param_summary_panel(p: dict):
    ...
    for key, title in (
        ...
        if bool(p.get(key, {}).get("active")):
            sections.append((title, [row("Aktív", "Igen")]))

    f = p.get("feldolgozas", {})
    if bool(f.get("active")):
        sections.append(
            ("Feldolgozás", [row("Elemzői munkaóra", f.get("elemzoi_munkaora"), "óra")])
        )
...
def _render_job_tabs(state: dict, state_key: str, is_editable: bool):
    ...
    for i, done in enumerate(complete_flags):
        ...
        css_rules.append(
            ...
            " font-weight:500 !important; line-height:1 !important; border-radius:0.3rem !important;"
...
def render_stage1_kalkulacio_trening(offer_id: int, is_editable: bool, db: Session):
    if not is_editable:
        st.info(
            "Ez a szakasz lezárult – a kalkuláció csak olvasható módban jelenik meg."
        )

    st.markdown("#### Kalkuláció – Tréning")
    st.caption(
    ...
    state_key = f"tren_kalk_{offer_id}"
    if state_key not in st.session_state:
        record = crud.get_stage1_kalk_trening(db, offer_id)
    ...
    if is_editable:
        ...
        if st.button("Kalkuláció mentése", type="primary", key=f"{state_key}_save"):
            ...
            if changed:
                crud.upsert_stage1_kalk_trening(db, offer_id, params_json)
                st.toast("Tréning kalkuláció mentve!")
                st.rerun()
            else:
                st.info("Nem történt változás a paraméterezésben – mentés kihagyva.")

    history = crud.get_kalk_history(db, offer_id, "trening")
    if history:
        with st.expander(f"📋 Kalkuláció-verziók ({len(history)} db)"):
            ...
            selected_label = st.radio(
                "Betöltendő verzió:", labels, index=0, key=f"hist_radio_{offer_id}_tren"
            )
            if st.button(
                "⬆️ Kiválasztott verzió betöltése",
                key=f"hist_load_{offer_id}_tren",
                disabled=not is_editable,
