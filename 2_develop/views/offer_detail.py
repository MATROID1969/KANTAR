...
def _render_stage1(offer, db):
    ...
    # A Kalkuláció fül csak akkor aktív, ha a kutatás típusa már be van állítva.
    tartalom = crud.get_stage1_tartalom(db, offer.id)
    kutatas_tipusa_megvan = bool(tartalom and tartalom.kutatas_tipusa)
    ...
    with sub_tabs[2]:
        if not kutatas_tipusa_megvan:
            st.warning(
                "A Kalkuláció fül csak akkor válik elérhetővé, ha a Tartalom fülön "
                "megadtad a Kutatás típusát."
            )
        else:
